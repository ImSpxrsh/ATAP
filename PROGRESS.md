...
| M3 | Confounder decomposition (MCL1, BCL2:BCL-XL ratio, BCL2 gatekeeper muts) | not started — nested models, ΔR²/ΔAIC of executioner state beyond confounders |
...
2. **M3 confounder decomposition** on the BeatAML cohort already in hand (highest-value: shows the
   M2 signal survives MCL1 / ratio / gatekeeper-mutation controls — or honestly that it doesn't).
3. **M1 formal executioner_loss_score** + **M4 specification curve** (the signature robustness panel).
